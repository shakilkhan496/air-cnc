...
const Signup = () => {
  ...
  const handleSubmit = e => {
    ...
    // bb83164738cd11b601b966eec915dec8
    const url = `https://api.imgbb.com/1/upload?key=${process.env.REACT_APP_imbb_api_key}`;
    fetch(url, {
      method: 'POST',
      body: formData,
    }).then(res => res.json())
            ...
            updateUserProfile(name, data.data.url)
              .then(
                () => {
                  ...
                  verifyEmail().then(toast.success('Please check your email and click the link to verify your account')).catch(err => console.log(err))
                }
              )
            console.log(user);
          })
